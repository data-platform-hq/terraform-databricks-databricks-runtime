resource "databricks_mount" "adls" {
  ...
  extra_configs = {
    ...
    "fs.azure.account.oauth2.client.secret" : "{{secrets/main/${data.azurerm_key_vault_secret.sp_key.name}}}"
    ...
    "spark.databricks.sqldw.jdbc.service.principal.client.secret" : "{{secrets/main/${data.azurerm_key_vault_secret.sp_key.name}}}"
  }
}
